...
import { getAdminFeedbacks, getAdminFeedback, replyAdminFeedback } from "@/lib/admin-api";
...
export default function AdminFeedbackPage() {
  ...
  const openDetail = async (item: Feedback) => {
    try {
      const detail = await getAdminFeedback(item.id);
  ...
  const handleReply = async () => {
    ...
    setReplying(true);
    try {
      await replyAdminFeedback(detailItem.id, { reply: reply.trim(), status: replyStatus });
