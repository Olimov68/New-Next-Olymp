...
export function TeamSection() {
  ...
  return (
    <section id="team" className="py-20 bg-background border-t border-border">
      <div className="container mx-auto px-4">
        <div className="text-center mb-14">
          ...
          <h2 className="text-3xl md:text-4xl font-bold text-foreground mb-4">{t("team.title") || "Bizning Jamoa"}</h2>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 max-w-5xl mx-auto">
          {team.map((member) => (
            <Card key={member.name} className="group hover:shadow-2xl hover:shadow-blue-500/10 hover:-translate-y-1 transition-all duration-300 border border-border bg-card backdrop-blur-sm rounded-2xl overflow-hidden">
              <CardContent className="p-6">
                <div className="flex items-center gap-4 mb-4">
                  <div className={`flex h-14 w-14 items-center justify-center rounded-2xl bg-gradient-to-br ${member.color} text-foreground font-bold text-lg shadow-lg`}>
                    {member.initials}
                  </div>
                  <div>
                ...
                <p className="text-sm text-muted-foreground leading-relaxed">{member.desc[lang]}</p>
              </CardContent>
            </Card>
          ))}
        </div>
