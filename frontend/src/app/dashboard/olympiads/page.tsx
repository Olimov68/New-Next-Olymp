...
export default function OlympiadsListPage() {
  const [olympiads, setOlympiads] = useState<AssessmentBase[]>([]);
  ...
  const filtered = useMemo(() => {
    return olympiads.filter((o) => {
      ...
      if (subjectFilter && o.subject !== subjectFilter) return false;
      if (statusFilter && o.status !== statusFilter) return false;
      if (paidFilter === "free" && !isFree(o)) return false;
      if (paidFilter === "paid" && isFree(o)) return false;
      if (gradeFilter && o.grade !== Number(gradeFilter)) return false;
      if (languageFilter && o.language !== languageFilter) return false;
      return true;
    });
  }, [olympiads, search, subjectFilter, statusFilter, paidFilter, gradeFilter, languageFilter]);
    ...
    <div className="space-y-6">
      ...
      {filtered.length === 0 ? (
        <div className="flex flex-col items-center py-16 text-muted-foreground">
          ...
          <p className="font-medium">Olimpiadalar topilmadi</p>
          <p className="text-sm mt-1 opacity-70">Boshqa filtrlarni sinab ko&apos;ring</p>
        </div>
      ) : (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {filtered.map((o) => (
            <AssessmentCard key={o.id} assessment={o} examType="olympiad" />
          ))}
        </div>
      )}
    </div>
