      ...
      <div className="container mx-auto px-4">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 max-w-5xl mx-auto">
          {testimonials.map((item) => (
            <Card
              key={item.name}
              className="group hover:shadow-2xl hover:shadow-blue-500/10 hover:-translate-y-1 transition-all duration-300 border border-border bg-card backdrop-blur-sm rounded-2xl overflow-hidden"
            >
              <CardContent className="p-6">
                ...
                <div className="flex items-center gap-3 pt-4 border-t border-border">
                  <div
                    className={`flex h-11 w-11 items-center justify-center rounded-full bg-gradient-to-br ${item.color} text-foreground font-bold text-sm shadow-lg`}
                  >
                    {item.initials}
                  </div>
                  <div>
