  ...
  return (
    <section className="relative py-24 overflow-hidden bg-background border-t border-border">
      <div className="pointer-events-none absolute inset-0" aria-hidden>
        ...
        <div className="absolute bottom-0 right-1/3 w-[500px] h-[500px] bg-indigo-500/5 rounded-full blur-3xl" />
      </div>
      ...
      <div className="container mx-auto px-4 relative">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-14 max-w-6xl mx-auto">
          {currentFeatures.map((feature, i) => (
            <div
              key={i}
              className="group rounded-2xl border border-border bg-card backdrop-blur-sm p-6 hover:bg-accent hover:border-blue-400/30 transition-all duration-300 hover:-translate-y-1"
            >
        ...
        <div className="flex flex-wrap justify-center gap-4">
          <Link href="/register">
            <Button size="lg" className="bg-gradient-to-r from-blue-500 to-indigo-600 hover:from-blue-600 hover:to-indigo-700 text-foreground shadow-lg shadow-blue-500/25 border-0 gap-2 px-8">
              {lang === "uz" ? "Imtihonni boshlash" : lang === "ru" ? "Начать экзамен" : "Start Exam"}
              <ArrowRight className="h-4 w-4" />
            </Button>
          </Link>
          <Button size="lg" variant="outline" className="border-border text-foreground hover:bg-accent backdrop-blur-sm gap-2 px-8">
